...
class MoveGroupInterface(object):
  ...
  def go_to_home_state(self):
    ...
    pub = self.pub
    pub.publish(True)
    joint_goal = [-1.5638910532,0.0138541124761,2.26129961014,-2.22698187828,1.54945480824,0.00317068165168]
  ...
  def move_item_cb(self, data):
    ...
    item_pose.orientation.x = 0.741980868361
    item_pose.orientation.y = -0.00373166490129
    item_pose.orientation.z = 0.000823110204693
    item_pose.orientation.w = 0.670410164118

    item_pose.position.y += 0.15
    item_pose.position.z += 0.09
    print item_pose
    item_pose.position.y += 0.05
    ...
    #item_movement = [0,0,0.15]
    #self.go_to_cartesian_goal(item_movement)
    self.go_to_home_state()
    #catch the item
    #self.go_to_end_position_3()


    '''item_movement = [0,0.10,0]
    self.go_to_cartesian_goal(item_movement)
    end_pose = geometry_msgs.msg.Pose()
    end_pose.position.y += 10
    self.go_to_pose_goal(self,end_position)
    item_movement = [0,-10,0] 
    self.go_to_cartesian_goal(item_movement)
    raw_input()
    '''
  ...
  def move_item(self):
    rospy.Subscriber("object_detection/item_position", geometry_msgs.msg.Pose, self.move_item_cb, queue_size=1)
    rospy.spin()
...
def main():
  try:
    ...
    #raw_input()
    _move_group_interface.go_to_home_state()
