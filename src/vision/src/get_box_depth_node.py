...
def box_cb(boxes_msg):
    ...
    flag = False
    try:
        depth_msg = rospy.wait_for_message('/camera/aligned_depth_to_color/image_raw', Image, timeout=0.5)
    ...
    for bbox in boxes_msg.bounding_boxes:
        if bbox.Class == TARGET_CLASS:
            ...
            if depth_array[ymid, xmid] is not None:
                print("depth of box center: {:.2f}mm".format(depth_array[ymid, xmid]))
            else:
                for i in range(-2, 2):
                    for j in range(-2, 2):
                        if depth_array[ymid+i, xmid+j] is not None:
                            print("depth of box center: {:.2f}mm".format(depth_array[ymid+i, xmid+j]))
                            flag = True
                            break
                    if Flag:
                        break
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("darknet_ros/bounding_boxes", BoundingBoxes, callback=box_cb, queue_size=1)
