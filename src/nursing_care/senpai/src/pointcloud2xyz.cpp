...
void u_v_CB(const std_msgs::Int16MultiArray::ConstPtr& input){
    int u=input->data[0];
    int v=input->data[1];
...
int main (int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
  ros::Subscriber coordinate_sub =nh.subscribe("object_detection/xy_coordinate", 1, u_v_CB);
  ros::Subscriber cloud_sub =nh.subscribe("/camera/depth_registered/points", 1, cloudCB);
  pub = nh.advertise<geometry_msgs::Pose> ("object_detection/item_position", 1);
  ros::spin ();
  return 0;
}
